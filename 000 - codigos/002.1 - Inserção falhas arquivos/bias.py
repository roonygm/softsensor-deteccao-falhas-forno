...
PASTA_DESTINO = Path(r"C:\Dados\Usina_7_Forno\003 - Dados CSV\01 - Dados Divididos 2h\00 - Cruzeiro\01 - Testes\01 - Falha tipo bias")
...
# =========================
# FUNÇÃO PARA APLICAR BIAS
# =========================
def aplicar_falha_bias_tc(df: pd.DataFrame, n_amostras: int = 2000, bias_valor: float = 10.0):
    df_mod = df.copy()
    ...
    # Aplica bias constante do início da falha até o final
    df_mod.loc[df_mod.index[idx_inicio:], 'TC'] = (
        df_mod.loc[df_mod.index[idx_inicio:], 'TC'] + bias_valor
    )
...
def plot_tc_comparacao(nome_arquivo, tc_original, tc_modificado, idx_inicio_falha, pasta_destino=None):
    ...
    if pasta_destino is not None:
        nome_png = Path(nome_arquivo).stem + "_TC_bias_comparacao.png"
...
def main():
    ...
    for i in range(1, 51):
        ...
        try:
            ...
            # Aplica bias persistente
            df_mod, tc_original, idx_inicio_falha = aplicar_falha_bias_tc(
                df,
                n_amostras=N_AMOSTRAS_FALHA,
                bias_valor=BIAS_VALOR
            )
